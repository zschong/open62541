...
processCompleteChunkWithoutChannel(UA_Server *server, UA_Connection *connection,
    ...
    case UA_MESSAGETYPE_OPN:
    {
        ...
        retval = UA_SecureChannel_addChunk(connection->channel, message);
        if(retval != UA_STATUSCODE_GOOD)
            break;

        UA_SecureChannel_processMessages(connection->channel, server,
...
processCompleteChunk(void *const application, UA_Connection *connection,
    ...
    return UA_SecureChannel_addChunk(connection->channel, chunk);
}
...
UA_Server_processBinaryMessage(UA_Server *server, UA_Connection *connection,
    ...
    UA_SecureChannel_persistChunks(connection->channel);
}
