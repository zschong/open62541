...
processOPNResponse(void *application, UA_Connection *connection,
                   UA_ByteString *chunk) {
    UA_Client *client = (UA_Client*) application;
    UA_StatusCode retval = UA_SecureChannel_addChunk(&client->channel, chunk);
    client->connectStatus = retval;
    if(retval != UA_STATUSCODE_GOOD) {
        UA_Client_disconnect(client);
        return retval;
    }
    UA_SecureChannel_processMessages(&client->channel, client, processDecodedOPNResponseAsync);
    
    if(client->state < UA_CLIENTSTATE_SECURECHANNEL)
        return UA_STATUSCODE_BADSECURECHANNELCLOSED;

    retval |= UA_SecureChannel_persistChunks(&client->channel);
