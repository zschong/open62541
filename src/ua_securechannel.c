...
deleteMessages(UA_SecureChannel *channel) {
    UA_Message *me;
    while((me = TAILQ_FIRST(&channel->messages))) {
        TAILQ_REMOVE(&channel->messages, me, pointers);
        deleteMessage(me);
    }
}
...
void
UA_SecureChannel_processMessages(UA_SecureChannel *channel, void *application,
    ...
    while((me = TAILQ_FIRST(&channel->messages))) {
        ...
        /* Process */
        processMessage(channel, me, application, callback);
...
static UA_StatusCode
addChunk(UA_SecureChannel *channel, const UA_ByteString *chunk) {
...
UA_StatusCode
UA_SecureChannel_addChunk(UA_SecureChannel *channel, const UA_ByteString *chunk) {
    ...
    UA_StatusCode retval = addChunk(channel, chunk);
    if(retval != UA_STATUSCODE_GOOD)
        UA_SecureChannel_deleteMembersCleanup(channel);
    return retval;
}
    ...
    while((me = TAILQ_FIRST(&channel->messages))) {
        while((cp = SIMPLEQ_FIRST(&me->chunkPayloads))) {
            if(cp->copied)
                continue;
            UA_ByteString copy;
            retval = UA_ByteString_copy(&cp->bytes, &copy);
            if(retval != UA_STATUSCODE_GOOD) {
                UA_SecureChannel_deleteMembersCleanup(channel);
                return retval;
            }
